...
namespace WebPodryd_System.Controllers
{
    ...
    public class ContractTemplateController : ControllerBase
    {
        ...
        public async Task<ActionResult<ContractTemplateDto>> CreateContractTemplate(CreateContractTemplateDto createDto)
        {
            try
            {
                Console.WriteLine($"Received create request: Name={createDto.Name}, Type={createDto.Type}, ContractTypeId={createDto.ContractTypeId}, WorkServicesCount={createDto.WorkServices?.Count}");
                ...
                // Автоматически устанавливаем Type на основе ContractTypeId
                if (string.IsNullOrEmpty(createDto.Type))
                {
                    createDto.Type = contractType.Code;
                    Console.WriteLine($"Auto-set Type to: {createDto.Type}");
                }

                // УБРАНА ПРОВЕРКА НА УНИКАЛЬНОСТЬ НАЗВАНИЯ
                // Теперь можно создавать несколько шаблонов с одинаковыми названиями для одного типа договора
        ...
        public async Task<IActionResult> UpdateContractTemplate(int id, UpdateContractTemplateDto updateDto)
        {
            try
            {
                ...
                if (updateDto.ContractTypeId.HasValue)
                {
                    ...
                    template.Type = contractType.Code; // Автоматически обновляем Type
                    Console.WriteLine($"Updated ContractTypeId to: {template.ContractTypeId}, Type to: {template.Type}");
                }

                // Если передан Type отдельно (для обратной совместимости)
                if (!string.IsNullOrEmpty(updateDto.Type))
                {
                    template.Type = updateDto.Type;
                    Console.WriteLine($"Updated Type to: {template.Type}");
                }
        ...
        public async Task<ActionResult<IEnumerable<ContractTemplateDto>>> GetTemplatesByContractType(int contractTypeId)
        {
            ...
            {
                Console.WriteLine($"Error getting templates by contract type {contractTypeId}: {ex.Message}");
                Console.WriteLine($"Stack trace: {ex.StackTrace}");
                return StatusCode(500, $"Internal server error: {ex.Message}");
            }
        }
